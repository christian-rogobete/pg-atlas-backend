...
stored payload per signal, and writes the merged payload plus ``pushed_at``.
...
async def persist_collected(
    ...
    # when this run's collection is the chronologically newest.
    incoming_is_newest = previous is None or (signals.collected_at or "") >= (previous.collected_at or "")
