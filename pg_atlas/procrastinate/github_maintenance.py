...
def declared_maintainer_logins(owner: str, repo: str) -> frozenset[str]:
    """
...
class _CloserRef(msgspec.Struct, omit_defaults=True):
...
def _is_declared_maintainer(actor: _ActorRef | None, declared_logins: frozenset[str]) -> bool:
...
def classify_issue_close_events(events: list[_TimelineNode]) -> list[dt.datetime]:
    """
...
def _get_maintenance_client() -> Github:
    ...
    The shared crawler client keeps PyGithub's default transport policy,
    which retries up to ten times and sleeps until the advertised rate-limit
    reset (up to an hour) for REST and GraphQL alike — underneath any caller,
    so the collector's request, time, and wait caps could neither see nor
    bound it. With ``retry=None`` every rate limit and transport failure
    surfaces immediately and the bounded handling here governs; the explicit
    per-request timeout bounds any single in-flight request. This client
    performs no requests at construction time.
    """
...
class _MaintenanceCollector:
    ...
    def __init__(self, owner: str, repo: str, *, now: dt.datetime) -> None:
        ...
        self._repo = repo
        self._now = now
        self._declared_maintainers = declared_maintainer_logins(owner, repo)
    ...
    async def _bounded_request[T](self, func: Callable[..., T], /, *args: Any) -> T:
        ...
        The await is cancelled when the remaining wall-clock budget runs out
        and becomes the ordinary time-cap halt, so a slow request can neither
        overrun the repository deadline nor mark a phase complete after it.
        The worker thread itself keeps running until the client's transport
        timeout bounds it.
        """
    ...
    async def _classify_issue(self, node: _IssueCohortNode, created_at: dt.datetime) -> ResponseItem:
        ...
        already settled the item within the interval — an out-of-interval
        close must NOT stop the search, because a later comment page can
        still hold a within-interval answer. When a within-interval close
        settles early, the recorded response time may be the close even if an
        unfetched later-page comment came earlier; that imprecision touches
        only the context median and is bounded by the interval.
        """
        ...
        close_times = classify_issue_close_events(node.timelineItems.nodes or [] if node.timelineItems is not None else [])
...
def _keep_better_signal[SignalT: (ResponsivenessSignal, BacklogSignal, ReleaseFallback)](
    ...
    switched off is as much an observation as a measured value — so an older
    run finishing late can neither weaken nor roll back a newer one. The
    kept signal retains its own ``as_of`` and parameters; the materializer's
    freshness policy decides how long it stays ranked. The ISO-8601
    ``as_of`` strings share one format and compare lexically.
    """
...
def merge_collected_signals(old: MaintenanceSignals | None, new: MaintenanceSignals) -> MaintenanceSignals:
    ...
    request count) come from the chronologically newer run, so an older run
    finishing late cannot roll back a newer overview observation; signals
    merge individually under ``_keep_better_signal``.
    """
...
async def _resolve_repo(session: AsyncSession, owner: str, repo: str) -> _ResolvedRepo:
    ...
    return _ResolvedRepo(repo_id=repo_id, canonical_id=canonical_id, releases=releases)
...
async def _persist_collected(
    ...
    The row is locked and re-read inside this transaction: the snapshot taken
    before the network phase can be minutes old, and merging against it would
    let an overlapping run's completed signals be overwritten by this run's
    weaker ones. The lock serializes concurrent persists (worker vs explicit
    CLI run); the merge itself keeps the newer of two complete signals.
    """
    ...
    # ``pushed_at`` pairs with the overview observation: it is written only
    # when this run's collection is the chronologically newest, so an older
    # run finishing late cannot replace a newer push observation.
    incoming_is_newest = previous is None or (collected.signals.collected_at or "") >= (previous.collected_at or "")

    values: dict[str, Any] = {"repo_metadata": repo_metadata_merge_expression(payload)}
    if incoming_is_newest:
        values["pushed_at"] = collected.pushed_at

    await session.execute(
        update(Repo).where(Repo.id == resolved.repo_id).values(**values).execution_options(synchronize_session=False)
    )
