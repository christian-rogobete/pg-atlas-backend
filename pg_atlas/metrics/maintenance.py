...
class ResponsivenessSignal(msgspec.Struct, omit_defaults=True):
    ...
    #: comparable to fractions measured under the same window, interval, and
    #: draft rule, so the materializer ranks a signal only when these match
...
class MaintenanceSignals(msgspec.Struct, omit_defaults=True):
    ...
    ``collected_at`` stamps the run and gates the freshness of the
    ``pushed_at`` column value; each signal carries its own ``as_of`` and
