...
def _responsiveness_signal_dict(
    ...
    *,
    ...
    A fraction measured under a different window, interval, or draft rule is
    not comparable to the current pool, so a parameter mismatch keeps the
    value as context, labeled with the parameters it was measured under.
    """
...
def _build_repo_profile(
    ...
    *,
    ...
    # A blocked fallback may feed the compute only where its values end up as
    # context anyway (the governing path below); against a usable deps.dev
    ...
    if fallback_block_reason is not None and fallback_governs:
        ...
        # (computed over whatever dates it holds) are context, never ranked.
        cadence_block["days_since_last_release"] = _scalar_entry(
            STATE_INCOMPLETE, value=cadence.days_since_last_release, reason=fallback_block_reason, as_of=fallback_as_of
        )
        cadence_block["median_gap_days"] = _scalar_entry(
        ...
        if cadence.state == STATE_OK and cadence.days_since_last_release is not None:
            cadence_block["days_since_last_release"] = _scalar_entry(
                STATE_OK, value=cadence.days_since_last_release, as_of=scalar_as_of
            )
        ...
        if cadence.state == STATE_OK and cadence.median_gap_days is not None:
            cadence_block["median_gap_days"] = _scalar_entry(STATE_OK, value=cadence.median_gap_days, as_of=scalar_as_of)
